<template>
  <q-page class="column items-center justify-evenly" padding>
    <header class="header">
      <div class="title">Auto gpt assist</div>
      <div class="description">Your AI assistant achieve your goals as much
        as possible.</div>
    </header>
    <q-card>
      <q-card-section>
        <span>Define your AI assistant</span>
        <q-input filled v-model="name" label="Name" :placeholder="demo.name" hint="Provide meaningful AI name"
          :hide-hint="true" />
        <q-input filled v-model="role" label="Role" :placeholder="demo.role" hint="Describe the role of your AI"
          :hide-hint="true" />
        <span>Major goals or questions of your AI assistant</span>
        <div class="goal" v-for="(goal, index) in goals" :key="`goal_${index}`">
          ...
          <q-icon size="1.6rem" color="green" name="remove_circle_outline" v-show="goals.length > 1"
    ...
    <q-dialog v-model="prompt" persistent>
      <q-card style="min-width: 350px">
        ...
        <q-card-section class="q-pt-none">
          ...
          Pinecone key        <input label="Pinecone key" type="password" />
          <br />
          pinecone environment<input label="pinecone environment" type="password" />

          <!--   <q-input dense v-model="Pinecone API key" label="Pinecone API key for vector database" type="password" />
          <q-input dense v-model="PineCone Environment" label="Pinecone Environment" type="password" /> -->
